...
<div class="max-w-3xl mx-auto bg-white shadow-md rounded-lg p-6">
    ...
    <div class="mb-6">
        ...
        <select wire:model="payment_method_id" id="payment_method" class="block w-full mt-1 p-3 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
    ...
    <div class="mb-6">
        ...
        <select wire:model="client_id" id="client" class="block w-full mt-1 p-3 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
                ...
                @foreach($cart as $index => $item)
                    <tr class="border-b border-gray-200">
                        ...
                        <td class="p-3 text-center">
                            <button wire:click="updateCart({{ $index }}, 'quantity', {{ $item['quantity'] - 1 }})" class="px-2 py-1 text-sm font-medium text-gray-700 bg-gray-200 rounded hover:bg-gray-300">-</button>
                            <input type="text" wire:model.lazy="cart.{{ $index }}.quantity" class="w-12 p-2 text-center border border-gray-300 rounded-lg mx-2">
                            <button wire:click="updateCart({{ $index }}, 'quantity', {{ $item['quantity'] + 1 }})" class="px-2 py-1 text-sm font-medium text-gray-700 bg-gray-200 rounded hover:bg-gray-300">+</button>
                        </td>
                        <td class="p-3 text-right text-gray-800">
                            <input type="text" wire:model.lazy="cart.{{ $index }}.unit_price" class="w-24 p-2 text-right border border-gray-300 rounded-lg">
                        </td>
                        <td class="p-3 text-right text-gray-800">
